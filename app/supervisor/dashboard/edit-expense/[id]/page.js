...
export default function SupervisorEditExpensePage({ params }) {
    ...
    return (
        <div style={{ minHeight: "100vh", position: "relative", zIndex: 1 }} className="responsive-root">
            ...
            <main className="page-content" style={{ position: "relative", zIndex: 10 }}>
                <Link href="/supervisor/dashboard/requests" style={{ display: "inline-block", marginBottom: "24px", color: "var(--primary)", textDecoration: "none", fontWeight: "600" }}>
                    ← Back to Requests
                </Link>
                ...
                {loading ? (
                    ...
                    <div className="glass-card fade-up" style={{ padding: "32px", textAlign: "center", color: "var(--danger)" }}>
                        <h3>Error</h3>
                        <p>{error}</p>
                        <button className="btn-primary" onClick={() => router.push("/supervisor/dashboard/requests")} style={{ marginTop: "16px", padding: "8px 16px", width: "auto" }}>Go Back</button>
                    </div>
                ) : (
                    <UnifiedExpenseBillForm onSuccess={() => router.push("/supervisor/dashboard/requests")} initialData={initialData} />
                )}
            </main>
        </div>
    );
